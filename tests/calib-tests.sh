...
root = sys.argv[1]
# (fixture, genuine review that MUST match, [decoys that must NOT match]). Decoys include
# the exact substring/common-word false-positives the consensus review named (stack TRACE,
# add a docSTRING, NONE of the inputs, qty: int | bool, "handle the n=0 case").
cases = [
 ("bugs/identity-compare",
  "Using `is` here compares object identity, not value — use == instead.",
  ["This function looks fine; it is a clean helper and the logic is correct."]),
 ("bugs/empty-average",
  "ZeroDivisionError when xs is empty: sum/len divides by zero for an empty list.",
  ["This division could be written more clearly with a helper.", "The division operator here is fine."]),
 ("bugs/negative-zero-slice",
  "items[-n:] with n==0 becomes items[0:], returning the whole list instead of an empty one.",
  ["Handle the n == 0 edge case in the docstring.", "Consider documenting behavior when n is 0.",
   "When n is zero, -0 is surprising; add a test for that edge case.", "The expression can produce -0 for n == 0; document that case."]),
 ("bugs/get-vs-setdefault",
  "out.get(key, []) returns a throwaway list; the dict stays empty because nothing is inserted.",
  ["Add type hints to pairs and a docstring."]),
 ("injection/unsafe-yaml",
  "yaml.load with UnsafeLoader on untrusted input allows arbitrary code execution — use safe_load.",
 ...
 ("payment-logic/int-truncation",
  "Integer division // drops the remainder cents; you must distribute the remainder.",
  ["Consider using proper rounding conventions when displaying currency to the user."]),
 ("payment-logic/missing-rounding",
  "The tax produces fractional cents that are never rounded to a whole-cent integer amount.",
  ["Use Decimal for better precision generally.", "Add an integer type hint."]),
 ("payment-logic/bankers-rounding",
  "round() does half-to-even, so 2.675 rounds to 2.67 not 2.68; the docstring promises half-up. Use Decimal with ROUND_HALF_UP.",
  ["Add a docstring example and round the result for display.",
   "The docstring says this is half-up rounding; add tests for that contract.",
   "Document the half-up behavior more clearly and add a unit test."]),
 ("payment-logic/discount-before-tax",
  "The docstring says discount the pre-tax price, but the code taxes the full price then subtracts — wrong order.",
  ["Rename the variables and add tests for the zero-discount case.",
   "Please clarify the pre-tax wording in the docstring.",
   "The pre-tax phrasing in the docstring could be clearer."]),
 ("perf/n-plus-one",
  "This runs one query per order — a classic N+1; batch into a single query or join.",
 ...
 ("type-drift/wrong-container",
  "Iterating a dict yields keys (the ids), so u[\"name\"] indexes an int and raises.",
  ["There is some type confusion in this module that could be clearer with annotations."]),
 ("type-drift/annotation-mismatch",
  "Annotated -> int but returns a str; parse_count returns a str so * 2 duplicates the string.",
  ["Please add a docstring to this function for clarity.", "The docstring is missing."]),
 ("type-drift/none-return",
  "find returns None implicitly and the caller calls .upper(), raising AttributeError.",
  ["None of the branches are tested.", "Add handling for nonempty inputs."]),
 ("type-drift/bool-is-int",
  "bool is a subclass of int, so isinstance(True, int) is True and set_quantity(True) sneaks through.",
  ["Add a type hint like qty: int | bool for clarity.", "Consider documenting the bool flag elsewhere."]),
 ("type-drift/optional-propagation",
  "db.get returns None when uid is absent, and that None propagates unchecked into format_name (TypeError).",
  ["None of the inputs are validated.", "None of these functions have docstrings.", "Handle the nonexistent and nonempty cases."]),
 ("concurrency/toctou",
  "check-then-act between exists() and mkdir() is a TOCTOU race condition; use exist_ok.",
  ["The stack trace should be logged when mkdir fails."]),
 ("concurrency/unguarded-counter",
  "count += 1 is a read-modify-write with no lock — a race condition with lost updates across threads.",
  ["Add a traceback to the error path; the stack trace is unhelpful."]),
 ("concurrency/check-then-act-cache",
  "The lock is released right after the membership check, so two threads can both build the value — a race condition.",
  ["Please fix the stack trace handling and add a traceback logger here.", "The trace output is noisy."]),
...
for fx, good, decoys in cases:
    ...
    for d in decoys:
        if match(d):     errs.append("%s: decoy matched (false positive): %r" % (fx, d))
if errs: [print("  -", e) for e in errs]; sys.exit(1)
PY
